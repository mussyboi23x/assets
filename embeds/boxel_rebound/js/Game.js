  ...
  function Game() {
    ...
    this.init = function () {
      ...
      document.addEventListener(
        "resume",
        function () {
          /*window.Game.resumeGame();*/ window.Game.playMusic();
          StatusBar.hide();
        },
        false
      );
      ...
      this.stage.on("stagemousedown", function (event) {
        window.Game.stageMouseDown(event);
      });
      this.stage.on("stagemousemove", function (event) {
        window.Game.stageMouseMove(event);
      });
      this.stage.on("stagemouseup", function (event) {
        window.Game.stageMouseUp(event);
      });
      ...
      this.assetManager.preload.on("progress", function () {
        window.Game.assetManager.updateLoading();
    ...
    this.setStage = function () {
      ...
      switch (this.view) {
        case 0:
          this.artboard.addChild(this.theme);
          break;
        case 1:
          this.artboard.addChild(this.theme, this.home);
          break;
        case 2:
          this.artboard.addChild(
            this.theme,
            this.levelMap,
            this.player,
            this.particles
          ...
          break;
        case 3:
          this.artboard.addChild(this.levelEditor);
          break;
      }
    ...
    this.stageMouseDown = function (event) {
      if (this.view == 0) {
        ...
        this.levelEditor.stageMouseDown(event);
      }
    };
    this.stageMouseMove = function (event) {
      if (this.view == 0) {
        ...
        this.levelEditor.stageMouseMove(event);
      }
    };
    this.stageMouseUp = function (event) {
      if (this.view == 0) {
        ...
        this.levelEditor.stageMouseUp(event);
      }
    };
      ...
      if (this.view == 1) {
        ...
        if (this.gamePaused == true) {
          if (window.Game.dialog.state == "complete") {
            if (
              window.Game.home.selectedLevel ==
    ...
    this.handleKeyDown = function (e) {
      ...
      switch (e.keyCode) {
        ...
        case 38:
          ...
          break; //up
        case 37:
          window.Game.levelEditor.leftKey();
          break; //left
        case 39:
          window.Game.levelEditor.rightKey();
          break; //right
        case 46:
          window.Game.levelEditor.deleteKey();
          break; //delete
        case 65:
          window.Game.levelEditor.leftKey();
          break; //a
    ...
    this.muteVolume = function () {
      ...
      window.storageManager.storeVolume(0);
    };
    this.unmuteVolume = function () {
      createjs.Sound.volume = 1;
      window.storageManager.storeVolume(1);
    };
    ...
    this.syncLocalVolume = function () {
      createjs.Sound.volume = window.storageManager.getVolume();
    };
    this.resetMultipliers = function () {
      this.speedMultiplier = 1;
      this.gravityMultiplier = 1;
    };
